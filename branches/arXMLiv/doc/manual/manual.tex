...
\lstdefinestyle{shell}{language=bash,escapechar=@,basicstyle=\ttfamily\small,%
   morekeywords={latexml,latexmlpost,latexmlmath,latexmls,latexmld,latexmlc},
...
%%%----------------------------------------------------------------------
\section{Optimized Processing}\label{usage.daemon}\index{latexmls!basic usage}
%TODO: Rethink index
...
The \ltxcmd{latexmld} daemon, as well as the
\ltxcmd{latexmls} and \ltxcmd{latexmlc} server-client setup facilitate such purposes. Note that these executables are in \textbf{beta
  stage} and are currently under heavy development. Notable lapses in the current beta stage are the unsupported  website and bibliography features, originally provided in \ltxcmd{latexml} and \ltxcmd{latexmlpost}, as well as missing some of the more advanced customization options relateed to graphics and scanning.

\begin{lstlisting}[style=shell]
  latexmls {options}
\end{lstlisting}

The \ltxcmd{latexmls} server invokes a TCP/IP server daemon on a designated port (a set of default ports is assumed). The server can be then queried via standard socket communication, which facilitates the input/output flow. The setup is highly customizable, both on startup and during runtime. Customizations include three modes of operation, output representation, destination and log targets\footnote{now with support for pattern specifications, such as \#dir/\#name.xhtml}, preloaded libraries, timeout and autovivification thresholds, etc. 


The \textit{math}, \textit{fragment} and \textit{standard} modes facilitate three types of processing jobs, with specially tuned optimizations. The \textit{math} mode operators analogously to \ltxcmd{latexmlmath}, expecting a single mathematical expression on input. The \textit{standard} mode is the analog to the standard \ltxcmd{latexml} processing. The new feature of \textit{fragment} processing is particularly useful to processing libraries of controlled {\LaTeX} syntax, such as ZentralBlatt Math's journals of mathematical reviews. Given a standard preamble, specified by a set of \code{--preload=\textit{package}} options, it becomes possible to avoid both Perl's and {\LaTeXML}'s reinitializations, making batch conversions of such documents faster by over a magnitude.


Two important options are \code{--local} and \code{--port=\textit{number}}, as they can be set only during the startup of the server. The \code{--local} switch permits the read/write use of the filesystem the server is running on, while by default (or by specifying \code{--nolocal}), it is forbidden access and can communicate only via the respective socket. Multiple {\LaTeXML} servers can coexist simultaneously, if started on different ports, which helps utilize the parallel computing capabilities of multi-core processors.


In order to enable out-of-the-box server use for external applications, the two-step conversion process of {\LaTeXML}-proper and postprocessing can be merged into a single step, via the \code{--post} trigger. Once enabled, it becomes possible to use the full range of options available to \ltxcmd{latexmlpost}.
%, with the additional \code{--keepTeX} switch, which preserves the original syntax of the formula as a parallel annotation.
...
The \ltxcmd{latexmlc} executable is a simple client interface to a local {\LaTeXML} server, invoked by \ltxcmd{latexmls}. It accepts the full set of options of the server. If no server is present when the client is invoked, it will start its own server with the default timeout and the default port for that mode, unless otherwise specified by the user.


If an active server is found on the designated port, its startup options are used for the conversion in the case that no options are specified by the client. All client options (except \code{--local} and \code{--port=\textit{number}}) take precedence and overwrite the startup options of the server during the runtime. The client can also be invoked without any options, in which case it will assume the default mode and port and start/connect to the respective server.

\begin{lstlisting}[style=shell]
  latexmld {options}
\end{lstlisting}

\textbf{Warning:} The \ltxcmd{latexmld} executable has been deprecated and will soon be rendered obsolete. It was the first prototype of a \LaTeXML daemon, that used pipes for IO. The follow-up \ltxcmd{latexmls} prototype based on sockets has proven more scalable and general. The pipe-based \ltxcmd{latexmld} daemon shares a subset of the processing options of \ltxcmd{latexmls} and is always out of date in terms of feature enhancements and bug fixes.
...
\index{Mouth (LaTeXML::)!architecture}%
\index{Gullet (LaTeXML::)!architecture}%
\index{Stomach (LaTeXML::)!architecture}%
\index{Token (LaTeXML::)!architecture}%
\index{Tokens (LaTeXML::)!architecture}%
\index{Definition (LaTeXML::)!architecture}%
\index{Expandable (LaTeXML::)!architecture}%
\index{Primitive (LaTeXML::)!architecture}%
\index{Box (LaTeXML::)!architecture}%
\index{List (LaTeXML::)!architecture}%
\index{Whatsit (LaTeXML::)!architecture}%
