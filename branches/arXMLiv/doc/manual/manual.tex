...
%%%----------------------------------------------------------------------
\section[Daemon]{Optimized Processing}\label{usage.daemon}\index{latexmls!basic usage}
%TODO: Add content , rethink index
When one is interested in processing jobs beyond a single document,
efficiency and convenience of processing are held in greater
value. Two such scenarios are processing jobs converting large
collections of documents, as well as multi-user web applications,
building on {\LaTeXML} as a core service.
...
A few examples of possible start-up invocations for setting up off-the-shelf services:
\begin{description}
\item[Formula Conversion Web Server]
\begin{lstlisting}[style=shell]
latexmls --mode=math --post --parallelmath --pmml --cmml --keepTeX --port=3334 --timeout=0 --autoflush=5000
\end{lstlisting}
\item[LaTeX Fragment Web Server]
\begin{lstlisting}[style=shell]
latexmls --mode=fragment --preload=LaTeX.pool --preload=article.cls
 --preload=amsmath.sty --preload=amsthm.sty --preload=amstext.sty --preload=amssymb.sty 
 --post --pmml --port=3344 --timeout=0 --autoflush=1000
\end{lstlisting}
\item[Document Conversion Web Server]
\begin{lstlisting}[style=shell]
latexmls --mode=standard --post --pmml  --port=3354 --timeout=0 --autoflush=500
\end{lstlisting}
\item[Batch conversion server for PlanetMath]
\begin{lstlisting}[style=shell]
...
\end{lstlisting}
\item[Batch conversion server for ZentrallBlatt Math]
\begin{lstlisting}[style=shell]
latexmls --mode=fragment --preload=zbl.cls
 --post --pmml --cmml --keepTeX --parallelmath
 --log=#dir/stderr.log
 --destination=#dir/#name.tex.xml 
 --path=./zbl/sty/
