...
The \ltxcmd{latexmld} daemon, as well as the \ltxcmd{latexmls} and \ltxcmd{latexmlc}
...
\ltxcmd{latexml} and \ltxcmd{latexmlpost} remain unsupported, furthermore some of the more
...
\code{--post} trigger. Once enabled, it becomes possible to use the full range of options
available to \ltxcmd{latexmlpost}.
...
The \ltxcmd{latexmlc} executable is a simple client interface to a local {\LaTeXML}
server, invoked by \ltxcmd{latexmls}.It accepts the full set of options of the server.
...
options (except \code{--local} and \code{--port=\textit{number}}) take precedence and
...
\textbf{Warning:} The \ltxcmd{latexmld} executable has been deprecated and will soon be
rendered obsolete. 
...
\ltxcmd{latexmls} prototype based on sockets has proven more scalable and general. The
pipe-based \ltxcmd{latexmld} daemon shares a subset of the processing options of
\ltxcmd{latexmls} and is always out of date in terms of feature enhancements and bug
fixes.
...
sequences for side effect or converting material
